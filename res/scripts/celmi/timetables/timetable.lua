...
function timetable.waitingRequired(vehicle)
    ...
    if not currentlyWaiting[tostring(currentLine)].stations[currentStop] then
    ...
    if timetableObject[tostring(currentLine)].stations[currentStop].conditions.type == "ArrDep" then
        ...
        if not (currentlyWaiting[tostring(currentLine)].stations[currentStop].currentlyWaiting[vehicle]) then
            ...
            if currentlyWaiting[tostring(currentLine)].stations[currentStop].outboundTime
               and (currentlyWaiting[tostring(currentLine)].stations[currentStop].outboundTime + 40) > time then
                return false
            end

            -- just arrived
            local nextConstraint = timetable.getNextConstraint(
                timetableObject[tostring(currentLine)].stations[currentStop].conditions.ArrDep, time)
            ...
            if timetable.beforeDepature(nextConstraint, time) then
                ...
                currentlyWaiting[tostring(currentLine)].stations[currentStop].currentlyWaiting[vehicle] = {
                    type = "ArrDep",
                    arrivalTime = time,
            ...
            -- already waiting
            local arivvalTime = currentlyWaiting[tostring(currentLine)].stations[currentStop]
                                            .currentlyWaiting[vehicle].arrivalTime
            local constraint = timetable.getNextConstraint(
                timetableObject[tostring(currentLine)].stations[currentStop].conditions.ArrDep, arivvalTime)
